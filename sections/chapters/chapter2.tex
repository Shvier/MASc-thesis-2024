...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item $b$ - \textit{the evaluation of $f(a)$}
        \item $\hat{b}$ - \textit{the evaluation of $\hat{f}(a)$}
    \end{itemize}
    \textit{They run the protocol as follows:}
    \begin{enumerate}
        \item $\prv$ \textit{computes the witness $w$ for $(a,b,\hat{b})$ such that}
...
\end{enumerate}
\begin{itemize}
    \item \textit{\textbf{Completeness}: It is clear that $w$ exists if and only if $f(a)=b$ and $\hat{f}(a)=\hat{b}$, which means $\vrf$ always accepts the proof if $\prv$ follows the protocol.}
    ...
    \begin{enumerate}
        ...
        \item \textit{$\mathcal{A}$ generates $w$ and $b^\prime$ at the evaluation point $a$.}
...
\textit{Let $\mathcal{S}$ generate $\{\omega^{*^\prime},\omega^{*^{\prime\prime}},\alpha^*,\beta^*\}$, and interpolate $f^*$ such that}
\[ f^*(\omega^{*^\prime})=\alpha^* \]
\[ f^*(\omega^{*^{\prime\prime}})=\beta^* \]
\[ f^*(x)=0,x\in{H} \]
\textit{Note} $\vrf$ \textit{can interact with $\mathcal{S}$ to execute the protocol and accept the proof from $\mathcal{S}$. Given $\{\omega^{*^\prime},\omega^{*^{\prime\prime}},\alpha^*,\beta^*\}$ are chosen uniformly at random each time,} $\vrf$ \textit{cannot distinguish between the transcript from $\mathcal{S}$ and the proof from} $\prv$. \textit{Therefore, the protocol has zero knowledge. We will use} $\open$ \textit{to denote this opening technique.}
